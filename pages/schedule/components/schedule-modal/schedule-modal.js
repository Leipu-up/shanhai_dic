    ...
    methods: {
        ...
        showModal(scheduleData = null) {
            ...
            if (scheduleData) {
                // 编辑模式
                isEdit = true;
                const selectedDate = scheduleData.pbrq;
                const staffOptions = this.data.staffOptions;
                formData = {
                    id: scheduleData.id,
                    selectedStaffId: scheduleData.jmygb.id,
                    selectedStaffName: scheduleData.jmygb.nickname,
                    scheduleType: scheduleData.pblx,
        ...
        selectDate(e) {
            ...
            if (isEdit) {
                // 编辑模式逻辑：只能选择一个日期
                if (index > -1) {
                    // 如果点击已选中的日期，则取消选中（清空选择）
                    selectedDates = [];
                } else {
                    // 如果点击未选中的日期，则替换为新的日期（只能选一个）
                    selectedDates = [date];
                }
            } else {
                // 新增模式逻辑：最多可以选择6个日期
        ...
        handleConfirm() {
            ...
            const scheduleData = {
                id: this.data.id,
                jmygb: {
                    id: this.data.selectedStaffId,
                },
                pblx: this.data.scheduleType,
                pbrq: this.data.selectedDates[0],
        ...
        validateForm() {
            ...
            const isValid = !!selectedStaffId && selectedDates.length > 0;
            this.setData({
                isFormValid: isValid
            });
            return isValid;
